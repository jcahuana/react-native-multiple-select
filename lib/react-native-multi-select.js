...
export default class MultiSelect extends Component {
  ...
  _displaySelectedItems = optionalSelctedItems => {
    ...
    const getDynamicWidth = (item, customWidth) => {
      if (item[displayKey].length * 8 + 60 >= customWidth) {
        return customWidth;
      }
      ...
      return (
        <View
          style={[
            ...
            widthSelectedItems ? {width: getDynamicWidth(item, widthSelectedItems)} : {width: item[displayKey].length * 8 + 60}
          ]}
          key={item[uniqueKey]}
        >
          <Text
